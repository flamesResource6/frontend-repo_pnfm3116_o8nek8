...
const NavLink = ({ href, children }) => (
  <a
    href={href}
    className="group relative text-sm md:text-base text-neutral-900 hover:text-neutral-700 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[#BDAE96] focus-visible:ring-offset-2 focus-visible:ring-offset-white rounded"
  >
    <span className="pb-1">{children}</span>
    <span className="pointer-events-none absolute left-1/2 -translate-x-1/2 -bottom-0.5 h-[2px] w-0 bg-[#BDAE96] transition-all duration-300 group-hover:w-full group-focus-visible:w-full" aria-hidden="true" />
  </a>
)
      ...
      <div id="mobile-menu" className={`${open ? 'block' : 'hidden'} md:hidden border-t border-neutral-200`}>
        <div className="px-4 py-4 space-y-3">
          {[
            ...
            <a key={item.label} href={item.href} className="block text-neutral-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#BDAE96] focus-visible:ring-offset-2 focus-visible:ring-offset-white rounded" onClick={() => setOpen(false)}>
              {item.label}
            </a>
          ))}
          <a
